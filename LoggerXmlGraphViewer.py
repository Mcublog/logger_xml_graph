...
def get_list_files(file_name):
    ...
    current_path =Path(__file__).parent.resolve().joinpath('data')
    dir_list = [d for d in current_path.iterdir() if d.is_dir()]
    file_list =[f for f in Path(dir_list[0]).iterdir() if f.is_file()]
...
def show_tph_graph(path: Path, type = 'Temperature'):
    tree = ET.parse(path)
    ...
    x = [i+1 for i in range(len(reports))]
    if type == 'Humidity':
...
def show_jrnl_graph(path: Path, type = 'Voltage'):
    tree = ET.parse(path)
    ...
    plt.xlabel("Time", fontsize = 8)
    # Generate a list from 0 to report number
    x = [i+1 for i in range(len(journal))]
    y_mean = 0
    if type == 'Humidity':
        set_plt_title_and_ylabel(plt, 'Humidity graphic', 'Humidity')
        ax.plot(x, hdc_hum, 'b-', label = 'hdc1080 humidity')
...
def show_graph(path:Path):
    if not path.is_file():
        return
    if 'acc_report' in path.name:
        show_acc_graph(path)
    if 'tph_report' in path.name:
        show_tph_graph(path)
    if 'jornal' in path.name:
